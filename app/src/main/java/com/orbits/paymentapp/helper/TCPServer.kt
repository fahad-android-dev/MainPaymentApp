...
class TCPServer(private val port: Int, private val messageListener: MessageListener) {
    ...
    fun sendMessageToAll(socket: Socket, message: String) {
        Thread{
            try {
    ...
    object WebSocketManager {
        ...
        fun getClientHandlers(clientIds: List<String>): List<ClientHandler?> {
            val clientHandlers = mutableListOf<ClientHandler?>()
            for (clientId in clientIds) {
                val clientHandler = clientHandlers[clientId.asInt()]
